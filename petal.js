...
const petalRotationStickToParent = ["Missile", "Dandelion", "Web", "Fire Missile", "Carrot", "Stalk"] //These petals are rotated to match to their rotation around parent flower
...
const petalRenderMap = {
    ...
    "Blood Jolt": (p) => {
        ...
        for(let i = 0; i<12; i++){
            let ang = i * Math.PI/6;
            ctx.lineTo(Math.cos(ang) * p.radius * 0.7, Math.sin(ang) * p.radius * 0.7)
            ctx.lineTo(Math.cos(ang + Math.PI/20) * p.radius * 1.4, Math.sin(ang + Math.PI/20) * p.radius * 1.4)
    ...
    "Jolt": (p) => {
        ctx.strokeStyle = blendColor('#c9c600', '#FF0000', blendAmount(p));
        ctx.fillStyle = blendColor('#fcf800', '#FF0000', blendAmount(p));
        if(checkForFirstFrame(p)){
            ctx.fillStyle = "#ffffff"; 
            ctx.strokeStyle = "#ffffff";
        }
